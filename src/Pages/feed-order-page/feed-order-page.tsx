...
const FeedOrderPage: FC = () => {
    ...
    useEffect(() => {
        dispatch({ 
            ...
            payload: location.pathname === `/feed/${id}` ? wsUrl : wsUrlToken
        });
        return () => {
        ...
        <>
        {
             !order ? (
                ...
                <div className={`${styles.container} `}>
                    ...
                    <span className={`text_type_main-medium mt-10 `}>   {order.name}  </span>
                        
                    <span className={`text_type_main-default mb-10  mt-4   ${order.status === 'done' && styles.done}`}>
                        { getStatus[order.status] } </span>


                    <div className={styles.ingredientsList}>
                        <span className="text_type_main-medium ">Состав:</span>
